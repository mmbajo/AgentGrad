...
class BaseMetricAccumulator(ABC):
    ...
    def _write_to_tensorboard(self) -> None:
        ...
        self._writer.add_scalar("Metrics/Episode Length", self._episode_length[-1], self._episode)
        self._writer.add_scalar("Metrics/Total Critic Loss", self._critic_losses[-1], self._episode)
        self._writer.add_scalar("Metrics/Total Actor Loss", self._actor_losses[-1], self._episode)
