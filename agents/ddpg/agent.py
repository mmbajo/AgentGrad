...
class DDPGAgent:
    ...
    def get_exploration_action(self, state: NDArray[np.float32]) -> NDArray[np.float32]:
        ...
        noise = np.random.normal(0, self.config.exploration_noise, size=self.config.action_dim)
        return np.clip(action + noise, self.config.action_low, self.config.action_high)  # Actions are typically normalized to [-1, 1]
